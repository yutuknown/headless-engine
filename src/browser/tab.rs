...
impl BrowserTab {
    ...
    pub async fn search_google(
        ...
        let url = match mode {
            Some("ai") | Some("udm50") => {
                format!("https://www.google.com/search?q={}&udm=50", encoded)
            }
            Some("web") | Some("udm14") => {
                format!("https://www.google.com/search?q={}&udm=14", encoded)
            }
            Some("images") | Some("udm2") => {
                format!("https://www.google.com/search?q={}&udm=2", encoded)
            }
    ...
    pub async fn google_flights_search(
        &mut self,
        origin: &str,
        dest: &str,
    ) -> Result<GenericGoogleResult> {
    ...
    pub async fn google_travel_explore(
        &mut self,
        destination: &str,
    ) -> Result<GenericGoogleResult> {
    ...
    pub async fn google_lens_visual_matches(
        &mut self,
        image_url: &str,
    ) -> Result<GenericGoogleResult> {
    ...
    pub async fn google_lens_exact_matches(
        &mut self,
        image_url: &str,
    ) -> Result<GenericGoogleResult> {
    ...
    pub async fn google_lens_about_image(
        &mut self,
        image_url: &str,
    ) -> Result<GenericGoogleResult> {
    ...
    pub fn google_capabilities(&self) -> Vec<&'static str> {
        vec![
            ...
            "youtube_channel",
            "youtube_playlist",
            "google_lens_visual_matches",
            "google_lens_exact_matches",
            "google_lens_products",
            "google_lens_about_image",
            "google_capabilities",
        ]
    }
    ...
    pub async fn click(&mut self, selector_or_text: &str) -> Result<Option<NavigationReport>> {
        ...
        let sel_json = serde_json::to_string(selector_or_text)
            .unwrap_or_else(|_| format!("\"{}\"", selector_or_text));
        let js_code = format!(
