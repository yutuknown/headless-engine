...
async fn main() -> Result<()> {
    ...
    let status_color = if nav_info.status == 200 {
        "\x1b[32m"
    } else {
        "\x1b[33m"
    };
    success!(
        "Navigation complete [Status: {}{}\x1b[0m | Size: \x1b[36m{:.2} MB\x1b[0m]",
        status_color,
        nav_info.status,
        nav_info.html_bytes as f64 / 1_048_576.0
    );
    ...
    success!(
        "Environment verified (Platform: \x1b[36m{}\x1b[0m, Webdriver: \x1b[36m{}\x1b[0m)",
        platform,
        webdriver
    );
    ...
    if let Some(search_results) = tab.extract_search_results() {
        if search_results.total_results_found > 0 {
            ...
            success!(
                "Extracted \x1b[36m{}\x1b[0m links and \x1b[36m{}\x1b[0m interactive forms",
                links.len(),
                forms.len()
            );
        }
    ...
    success!(
        "Payload compressed to \x1b[36m{}\x1b[0m bytes (\x1b[32m~{:.1}%\x1b[0m reduction)",
        markdown.len(),
        compression
    );
    ...
    success!(
        "iOS spoof complete [Status: \x1b[32m{}\x1b[0m]",
        ios_nav.status
    );
