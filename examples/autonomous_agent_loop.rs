...
async fn main() -> Result<()> {
    ...
    tab.navigate("https://en.wikipedia.org/wiki/Main_Page").await?;
    ...
    if let Some(target_link) = obs.interactive_elements.iter().find(|e| e.tag == "a" && e.text.contains("article")) {
        println!("\n[Step 3: Act] Agent decides to click element [{}] (Text: \"{}\")...", target_link.index, target_link.text);
