...
async fn main() -> Result<()> {
    ...
    tab2.navigate("https://www.google.com/search?q=Rust+programming+language+features&udm=50").await?;
    tokio::time::sleep(tokio::time::Duration::from_millis(500)).await;
    if let Some(shot) = tab2.screenshot_async().await {
        println!("  -> Google AI Mode Screenshot Bytes: {}", shot.png_bytes.len());
        if !shot.png_bytes.is_empty() {
            fs::write(evidence_dir.join("google_aimode_screenshot.png"), &shot.png_bytes)?;
            fs::write(artifact_dir.join("google_aimode_screenshot.png"), &shot.png_bytes)?;
        }
    }
    ...
    tab3.navigate("https://www.google.com/search?q=headless+browser+pure+rust+engine").await?;
    tokio::time::sleep(tokio::time::Duration::from_millis(500)).await;
    if let Some(shot) = tab3.screenshot_async().await {
        println!("  -> Google Normal Search Screenshot Bytes: {}", shot.png_bytes.len());
        if !shot.png_bytes.is_empty() {
            fs::write(evidence_dir.join("google_normal_search_screenshot.png"), &shot.png_bytes)?;
            fs::write(artifact_dir.join("google_normal_search_screenshot.png"), &shot.png_bytes)?;
        }
    }
