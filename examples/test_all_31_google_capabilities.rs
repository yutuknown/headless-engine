...
async fn main() -> anyhow::Result<()> {
    ...
    let res = tab.google_search("headless browser pure rust engine").await?;
    fs::write(evidence_dir.join("test_google_search.md"), res.to_markdown())?;

    println!("2. Testing youtube_search...");
    let res = tab.youtube_search("rust programming").await?;
    fs::write(evidence_dir.join("test_youtube_search.md"), res.to_markdown())?;
    
    println!("3. Testing google_ai_mode...");
    let res = tab.google_ai_mode("explain quantum computing").await?;
    fs::write(evidence_dir.join("test_google_ai_mode.md"), res.to_markdown())?;

    println!("4. Testing google_autocomplete...");
    let res = tab.google_autocomplete("rust").await?;
    fs::write(evidence_dir.join("test_google_autocomplete.md"), res.to_markdown())?;
