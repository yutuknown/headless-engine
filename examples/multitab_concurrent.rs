...
async fn main() -> Result<()> {
    ...
    let r1 = engine.get_tab_mut(&tab1_id).unwrap().navigate("https://en.wikipedia.org/wiki/Artificial_intelligence").await?;
    println!("  -> Tab 1 Title: {}", r1.page_title);

    println!("[Tab 2: Safari Mac] Navigating to Hacker News...");
    let r2 = engine.get_tab_mut(&tab2_id).unwrap().navigate("https://news.ycombinator.com/").await?;
    println!("  -> Tab 2 Title: {}", r2.page_title);

    println!("[Tab 3: iPhone iOS] Navigating to GitHub Explore...");
    let r3 = engine.get_tab_mut(&tab3_id).unwrap().navigate("https://github.com/trending").await?;
    println!("  -> Tab 3 Title: {}", r3.page_title);

    println!("\n>>> Active Tabs List:");
    for tab_info in engine.list_tabs() {
        println!("  - ID: {:<8} Profile: {:<15} URL: {:?}", tab_info.id, format!("{:?}", tab_info.profile), tab_info.url);
    }
